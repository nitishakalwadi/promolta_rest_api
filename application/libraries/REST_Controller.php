...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        {
            if ( ! ($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->where($this->config->item('rest_key_validity_column').'>', now())->get($this->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }
